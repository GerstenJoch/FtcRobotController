...
@Config
@Autonomous
public class MidHighConeLeft extends LinearOpMode
{
    ...
    public void runOpMode()
    {
        ...
        while (!isStarted() && !isStopRequested())
        {
            ...
            Forward(120,0,0.4,10);
            RotateLeft(323,0.4);//was 325
            Backward(20,323,0.3,2);
            goToLiftPosition(MidJunction-100);
            openIntake(); sleep(300);
            int GoForward = 77;
            int delay = 2;
            for (int i = 0; i < 2;i++) {
                Forward(13,325,0.3, 10);
                intake.swivel.setPosition(1);
                RotateLeft(270,0.4);
                new Thread(() -> goToLiftPosition(LowJunction)).start();
                if(i>0){GoForward=140;delay=3;}
                Forward(GoForward,270,0.6, delay);
                sleep(500);
                liftDownUntilSwitchPressedAndGetCone();
                goToLiftPosition(LowJunction);
                intake.swivel.setPosition(1);
                new Thread(() -> goToLiftPosition(HighJunction)).start();
                Backward(127,270,0.6,10);
                RotateRight(325,0.4);
                intake.swivel.setPosition(0);
                Backward(19,325,0.3, 3);
                goToLiftPosition(HighJunction-100);
                openIntake(); sleep(300);
            }
            Forward(17,323,0.3, 10);
            RotateRight(0,0.4);
            intake.swivel.setPosition(1);
    ...
    public void liftDownUntilSwitchPressedAndGetCone(){
        ...
        sleep(SleepAfterCmd);
        while(!digitalTouch.getState() && opModeIsActive() && (-1*arm.lift_encoder.getCurrentPosition()>100)){
